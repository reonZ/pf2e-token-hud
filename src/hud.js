...
import { RANKS, getCoverEffect, getUniqueTarget, localeCompare } from "./shared.js";
...
export class HUD extends Application {
    ...
    #checkIfObserved(token) {
        ...
        if (game.user.isGM && getSetting("key-holding") === "half" && !this.#holding)
            isObserved = false;
        else if (actor.isOfType("familiar") && !actor.master) isObserved = false;
        else
            isObserved =
                token.isOwner ||
                (getSetting("observer") && (token.observer || (isParty && getSetting("party"))));
    ...
    async getData() {
        ...
        if (showDistance === "all" || (showDistance === "self" && this.#isObserved)) {
            ...
            const unit = unitSplit[1]?.trim() || game.system.grid.units;
        ...
        if (!this.#isObserved || getSetting("see-status")) {
            ...
            if (statuses.length && hp.max) {
                ...
                const ratio = Math.clamp(current / max, 0, 1);
                const pick = (() => {
        ...
        const mainSpeed = (() => {
            ...
            if (selectedSpeed !== undefined) index = selectedSpeed;
                ...
                index = speeds.reduce(
                    (prev, { value }, index) => (value > prev.value ? { index, value } : prev),
                    base
                ).index;
            }
        ...
        return {
            ...
            hasNotes:
                !isCharacter &&
                (system.details.publicNotes || (system.details.privateNotes && isOwner)),
        };
    }
    ...
    activateListeners(html) {
        ...
        html.find("[data-action=toggle-hero]").on("click contextmenu", (event) => {
            ...
            const newValue = Math.clamp(value + change, 0, max);
        ...
        html.find("[data-action=toggle-dying], [data-action=toggle-wounded]").on(
            "click contextmenu",
            (event) => {
                event.preventDefault();

                const condition =
                    event.currentTarget.dataset.action === "toggle-dying" ? "dying" : "wounded";
    ...
    async #openSidebar(sidebarType, filter) {
        const type =
            typeof sidebarType === "string" ? sidebarType : sidebarType.currentTarget.dataset.type;
        ...
        const toggles = (() => {
            ...
            return Object.values(this.actor.synthetics.toggles).flatMap((domain) =>
                Object.values(domain).filter((toggle) => toggle.placement === placement)
            );
        })();
