...
function makeRepostHtml(target, defaultVisibility) {
	const flavor =
		target.attributes.getNamedItem("data-pf2-repost-flavor")?.value ?? "";
	const showDC =
		target.attributes.getNamedItem("data-pf2-show-dc")?.value ??
		defaultVisibility;
...
function repostAction(target, foundryDoc = null) {
	if (
		!["pf2Action", "pf2Check", "pf2EffectArea"].some((d) => d in target.dataset)
	) {
		return;
	}
 ...
 */
export function listenInlineRoll(html, doc) {
...
function resolveActor(foundryDoc, anchor) {
	...
	if (
		isInstanceOf(foundryDoc, "ItemPF2e") ||
		isInstanceOf(foundryDoc, "ChatMessagePF2e")
	)
