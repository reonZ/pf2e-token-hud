import { getItemWithSourceId, unownedItemToMessage } from "module-api";
...
const FORCED_ENTRY_UUID = "Compendium.pf2e.feats-srd.Item.uwJQUFLymAWtJu1a";
                ...
                modifiers: [
                    {
                        condition: (actor) =>
                            ...
                            ) && !getItemWithSourceId(actor, FORCED_ENTRY_UUID, "feat"),
                        modifiers: [
                            {
                                slug: "no-crowbar",
                                modifier: -2,
...
export function getSkillLabel(slug) {
    return game.i18n.localize(
        slug === "perception" ? "PF2E.PerceptionLabel" : CONFIG.PF2E.skillList[slug]
    );
}
...
export async function getSkillsData({ actor, filter }) {
    ...
    const noUntrained =
        ...
        !actor.itemTypes.feat.some((feat) => feat.sourceId === UNTRAINED_IMPROVISATION);
    ...
    for (let i = 0; i < SKILLS.length; i++) {
        ...
        const actionsList = actions
            .filter(
                ({ condition, trained }) =>
                    (!noUntrained || !isCharacter || !trained || actor.skills[slug].rank >= 1) &&
    ...
    skills.sort((a, b) =>
        a.slug === "perception" ? -1 : b.slug === "perception" ? 1 : localeCompare(a.name, b.name)
    );
