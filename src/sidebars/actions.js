import { createSelfEffectMessage, eventToRollMode, getActionIcon, objectHasKey } from "module-api";
...
export async function getActionsData({ hud, actor, filter }) {
    ...
    const stances = (await getStancesModuleApi()?.getStances(actor))?.sort((a, b) =>
        ...
        ? (
              await Promise.all(
                  blast.configs.map(async (config) => {
                      const damageType =
                          config.damageTypes.find((damage) => damage.selected)?.value ?? "untyped";
...
export function addActionsListeners({ el, actor, hud }) {
    ...
    action("toggle-versatile", async (event) => {
        ...
        const selection =
            target.classList.contains("selected") || value === baseType ? null : value;
