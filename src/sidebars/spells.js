...
import { MODULE_ID, getSetting, localize, modifier, templatePath } from "../module.js";
...
export function addSpellsListeners({ el, actor, hud }) {
    ...
    el.find("[data-action=cast-spell]").on("click", (event) => {
        ...
        const { castRank, slotId, entryId, itemId } = event.currentTarget.closest(".spell").dataset;
    ...
    el.find("input[data-is-staff=true]").on("change", (event) => {
        const value = event.currentTarget.valueAsNumber;
