...
export async function rollTreatWounds(actor, options) {
    ...
    if (hasFeat(actor, NATURAL_MEDICINE) && actor.skills.nature.rank) skills.push(actor.skills.nature)

    if (skills.length < 1) {
        ui.notifications.warn(localize('actions.treat-wounds.no-proficiency', { name }))
        return
    }
